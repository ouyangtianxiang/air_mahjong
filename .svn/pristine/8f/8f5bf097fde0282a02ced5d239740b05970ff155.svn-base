...
function echo_i() { echo -e "\033[32m$@\033[0m" ; }
function echo_w() { echo -e "\033[33m$@\033[0m" ; }
function echo_e() { echo -e "\033[31m$@\033[0m" ; }
function echo_q() { echo -e "\033[35m$@\033[0m" ; }
function echo_t() { echo "$(date), $@" ; }
	...
	function delcache() {
		...
		if [ -d fdb ] || [ -d online ] ;then
			rm -rf fdb/* online/*
			[ $? -eq 0 ] && { echo_i "delete cache file ok" && return 0; } || return 1
		else
			return 2
		fi	
	}
	...
	function checkstatus() {
		...
		checkservice && echo_i "the server was running" || { echo_q "the server was not running" && return 1; }
	...
	function startserver() {
		...
		checkservice && { echo_q "the server was aleady running" && exit 2; }
		...
		debug)
			...
			sleep 1
			checkservice && echo_i "start server success!" || { echo_q "start server faild!" && return 1; }
	...
	function useage() {
		...
		echo_i "sh $0 stop | start | status | delcache"
	}
	...
	stop)
		...
		checkservice || { echo_q "the server was not running" && exit 1; }
		stopservice
		while checkservice 
		do
		echo -n "."
		...
		[ $? -eq 0 ] && echo_i "stop server success!" 
