...
function Vector3.Slerp(from, to, t)
	...
	elseif cosom < -1 + 1e-6 then		
        ...
        if isnan(v.x) then v.x = 0 end;
	    if isnan(v.y) then v.y = 0 end;
	    if isnan(v.z) then v.z = 0 end;
	...
	if isnan(v2.x) then v2.x = 0 end;
	if isnan(v2.y) then v2.y = 0 end;
	if isnan(v2.z) then v2.z = 0 end;
