...
public static class LuaConst
{
	public static string luaDir {
		get {
			if (Application.platform == RuntimePlatform.WindowsEditor) { 
				return GameMain.it.luaPath + "src";
			} else {
				return Application.persistentDataPath + @"/src";            //lua逻辑代码目录
			}
		}
	}
	...
	public static string toluaDir {
		get {
			if (Application.platform == RuntimePlatform.WindowsEditor) {
				return GameMain.it.luaPath + "tolua";
			} else {
				return Application.persistentDataPath + @"/tolua";        //tolua lua文件目录
			}
		}
	}
