...
TopRankItem = class(Node)

TopRankItem.ctor = function(self, data)
	if not data then
		return;
	end
	...
	--设置点击头像事件
	 ...
	 --设置胜负信息
	 publ_getItemFromTree(self.listItem, {"item_view","text_score"}):setText(trunNumberIntoThreeOneFormWithInt(winmoney,false) .. "金币");

	 --设置等级
	 publ_getItemFromTree(self.listItem, {"item_view","text_lv"}):setText(level);
...
function TopRankItem.getSelfVipImgTag( self,vip_level )
