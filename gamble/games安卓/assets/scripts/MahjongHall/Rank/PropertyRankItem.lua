...
local rankListItem3 = require(ViewLuaPath.."rankListItem3");

PropertyRankItem = class(Node)

PropertyRankItem.ctor = function(self, data)
	if not data then
		return;
	end
	EventDispatcher.getInstance():register(NativeManager._Event, self, self.nativeCallEvent);

	self.listItem = SceneLoader.load(rankListItem3);
