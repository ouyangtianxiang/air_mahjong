...
local function set_breakpoint(file, line)
	...
	file = gsub(file, "\\", "/")
	local b,e = string.find(file,"Resource/scripts/");
	file = string.sub(file,e+1,-1);
	--print("setbreakpoint : " .. file .. " " .. line)
...
local function debug_exec(str1, str2,server)
      while true do
        local params, err = str1 
        if not params then
		  server:send("Error: Debugger connection "..(err or "error").."endeval\n");
          print_string("Error: Debugger connection "..(err or "error").."endeval\n")
          break
        ...
        if status == "200" then 
          ...
          if len > 0 then
            ...
            if err then
			  ...
			  server:send("Error in processing results: " .. err.."eval\n");
              print_string("Error in processing results: " .. err.."eval\n")
			  break
              --return nil, nil, "Error in processing results: " .. err
            end
			
			local str =  unpack(res) 
			if str then 
            print_string(tostring(str))
            server:send("eval:")
                while true do
                    if #str >4096 then
                        local strtemp =  string.sub(str,0,4096)
                        server:send(strtemp.."\n");
                        if #str - 4096 >4096 then                                      
                            str = string.sub(str,4096,string.len(str)-1-4096);
                        else
                            server:send(string.sub(str,0,string.len(str)-1));
                            break;
                        end
                    else
                        server:send(tostring(str).."\n");
                        break;
                    end
                end
                server:send("eval\n");
        ...
        elseif status == "401" then
          --len = tonumber(len)
          local res = str2--client:receive(len)
          print_string("Error in expression: " .. res)
		  server:send("Error in expression: " .. res.."eval\n");
		  break;
          --return nil, nil, res
        else
          print("Unknown error")
		  server:send("Error:Unknown error".."eval\n");
          --print_string("Error:Unknown error".."eval\n")
		  break;
    ...
    elseif command == "EXEC" then
      ...
      if chunk then 
        ...
        if status then                                                   
          ...
          --server:send(res)
          print_string(tostring(#res))
		  --if(#res >280000) then 
			--server:send("eval\n")
		  --else
		   debug_exec("200 OK " .. tostring(#res) .. "\n",res,server)
		  --end 
        else
          ...
          --server:send(res)
		  debug_exec("401 Error in Expression "  .. "\n",res,server)
        end
      else
        server:send("400 Bad Request eval\n")
      end
        ...
        if ok then
			...
			for k,v in pairs(vars) do 
				...
				for _,vv in pairs(v) do 
					if vv[2] and vv[4] then
						l = "stack: " .. vv[2] .." " .. vv[3].. " " .. vv[4];
						server:send(l)
					end
				end
			end
...
local function receiveFiles(fileName, str,name)

	local path = fileName..name..".zip"
	--print_string('jackie path: ' .. path)
	io.flush()
...
local function restartVM(filename)
	to_lua("main.lua");
	print_string("restart over")
end  

local function getStorageUpdatePath ()
...
local function start(controller_host, controller_port)
  ...
  controller_port =  mobdebug.port
print_string(controller_host..controller_port);
  local err
...
--  end
   ...
   if server then
        while true do
            local start = server:receive()
            if start then
                if start ~= "Reconnect" and  start then
                    print_string("start reveive pack ")
                    local filecount = server:receive()
                    print_string("receive pack length:" .. filecount)
                    filecount = string.gsub(filecount, "@", "")
                    if filecount then
                        filecount = tonumber(filecount);
                        local filetemp = ""
                        while true do
                            filecount = filecount - 4096;
                            if filecount > 0 then
                                local temp = server:receive(4096);
                                if temp then
                                    filetemp = filetemp .. temp
                                end
                            else
                                local temp = server:receive(filecount + 4096);
                                filetemp = filetemp .. temp
                                if filetemp ~= "" then
                                    break;
                                end
                        ...
                        print_string("receive pack over and pack length:" .. tostring(string.len(filetemp)))
                        receiveFiles(getStorageUpdatePath(), filetemp, start);
                        print_string("update over")
                        filetemp = "";
                        server:close();
